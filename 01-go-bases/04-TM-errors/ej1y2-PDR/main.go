...
func LeerDatosEmpleado() *os.File {
	defer func() {
		err := recover()
		if err != nil {
			fmt.Println(err)
	...
	file, ErrLectura := os.Open("customers.txt")
	if ErrLectura != nil {
...
Un estudio contable necesita acceder a los datos de sus empleados para poder realizar distintas liquidaciones. Para ello, cuentan con todo el
detalle necesario en un archivo .txt.
